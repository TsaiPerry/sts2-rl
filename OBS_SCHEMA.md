...
> **v8 (v14 mechanics-exposure, task 3, 2026-08-15, `OBS_SCHEMA_VERSION`
> 7 → 8):** `hand.f` grows 29 → 31 floats (see §5.2's row for the full
> field-29/field-30 writeup). No other combat segment resized.
...
| `hand.f` | 10 × 31 | **correction:** width is **31**, not left open-ended — the existing 24 card features (fields 0..23, unchanged) plus 5 R2 fields: affliction amount, `exhaust_on_next_play`, `_has_single_turn_retain`, `_has_single_turn_sly`, and `base_replay_count` (the plan-stage draft named only 4 of these 5; `base_replay_count` is the 5th, added because the R2 census lists it among the player-visible per-instance state missing from the observation and this schema bump is paid once), plus 2 v14 fields (schema 8, task 3, 2026-08-15): **field 29 `glow_gold`** — `CardModel.ShouldGlowGold` (`card.should_glow_gold(ctx)`), 0/1, the ONLY obs carrier for on_play-only conditions like Pact's End's exhaust-pile-≥3 check that no other field expresses; and **field 30 `block_preview_move`** — `previews.preview_card_block(s, card, props=ValueProp.MOVE)` / `ABS_SCALE`, the full MOVE-pipeline block preview (Dexterity, Frail, enchantments, Fasten), 0 when the card grants no block. Field 21 (`eff_block`) deliberately keeps its `ValueProp.NONE` parity value — see its own row entry above and `card_features`'s field-21 comment (`full_env.py`) — field 30 is the new, non-parity-constrained carrier for the hook-modified number. |
...
> schema 10.
>
> **Correction (2026-08-15, `RUN_OBS_SCHEMA_VERSION` 11 → 12, task 3, v14
> mechanics-exposure):** follows `full_env.OBS_SCHEMA_VERSION` 7 → 8 in
> lockstep — the embedded combat `hand.f` block grows 29 → 31 floats per
> row (§5.2's new field-29/field-30 entries), and this env's `MAX_HAND`
> (10) hand rows widen with it: **`f_dim` = 4735** floats, **`i_dim` = 1469**
> ints (unchanged — no new id columns). No checkpoint migration function
> exists for this bump.
