# Relic content audit — batch 4 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b04 -b audit-relic-b04 audit-relic
```

---

You are executing **batch 4** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b04` (branch `audit-relic-b04`)
...
2. `audit/tools/PROMPT.md` (**v4**) — the versioned instruction sheet and the
...
- **`daughter_of_the_wind`** — Named witness in `audit/records/seam/hook_dispatch.json` G8 (executed). Cite and match.
- **`dingy_rug`** — Sweep C stub (`ModifyCardRewardCreationOptions` — no matching base hook; needs a new one, so a larger fix).
- **`dollys_mirror`** — Sweep C stub: `AfterObtained` -> `Relic.after_obtained` IS dispatched (`run.py:552`). Premise false -> gap, not waiver.
- **`dragon_fruit`** — Sweep B: overrides `IsAllowed` = `IsBeforeAct3TreasureChest` (unmodelled — the sim has no `is_allowed` at all). One of the 16-relic cluster.
- **`dusty_tome`** — Sweep D: UNGUARDED `Card.upgrade()` at `dusty_tome.py:50` in `after_obtained`. 35 ported cards have `max_upgrade_level == 0`. Settle live/dormant by execution.
...
- `audit/tools/relic_probes_b04.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-04-lessons.md` — your report
...
`audit/tools/relic_probes_b04.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b04.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-04-lessons.md`
...
- Commit on `audit-relic-b04` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b04.py .superpowers/sdd/relic-batch-04-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-04-lessons.md` containing: the 15
