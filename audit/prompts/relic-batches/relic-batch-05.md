# Relic content audit — batch 5 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b05 -b audit-relic-b05 audit-relic
```

---

You are executing **batch 5** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b05` (branch `audit-relic-b05`)
...
**None** — no pool-wide sweep flagged any unit in this batch. Audit all 15 cold.
...
- `audit/tools/relic_probes_b05.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-05-lessons.md` — your report
...
`audit/tools/relic_probes_b05.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b05.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-05-lessons.md`
...
- Commit on `audit-relic-b05` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b05.py .superpowers/sdd/relic-batch-05-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-05-lessons.md` containing: the 15
