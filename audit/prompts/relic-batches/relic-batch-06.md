...
- **`fiddle`** — `audit/records/seam/hook_dispatch.json` step 35: the sole `AfterPreventingDraw` implementer, presentation-only body.
...
- `audit/tools/relic_probes_b06.py` — **your own** probe module (see below)
...
`audit/tools/relic_probes_b06.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b06.py`. Do NOT register anything in the
...
5. `git add audit/records/relic audit/tools/relic_probes_b06.py .superpowers/sdd/relic-batch-06-lessons.md`
