# Relic content audit — batch 7 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b07 -b audit-relic-b07 audit-relic
```

---

You are executing **batch 7** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b07` (branch `audit-relic-b07`)
...
- **`horn_cleat`** — Named witness in `audit/records/seam/turn_structure.json` G1 (an `AfterBlockCleared` listener). Cite and match.
- **`iron_club`** — Sweep A flagged this for a SECOND LOOK (`cards_played`).
- **`joss_paper`** — Sweep A candidate (`_ethereal_pending`, `cards_exhausted`; C# resets at `AfterCombatEnd`). Settle by execution.
- **`juzu_bracelet`** — Sweep B: `IsAllowed` unmodelled (16-relic cluster). Also a Sweep C stub — its dropped `ModifyUnknownMapPointRoomTypes` HAS a live dispatch site (`run.py`).
- **`kaleidoscope`** — Sweep B: `IsAllowedAtNeow` matches (`is_allowed_at_neow=False`). Also a Sweep C stub (`AfterObtained` is dispatched).
...
- `audit/tools/relic_probes_b07.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-07-lessons.md` — your report
...
`audit/tools/relic_probes_b07.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b07.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-07-lessons.md`
...
- Commit on `audit-relic-b07` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b07.py .superpowers/sdd/relic-batch-07-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-07-lessons.md` containing: the 15
