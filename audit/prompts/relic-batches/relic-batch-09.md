# Relic content audit — batch 9 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b09 -b audit-relic-b09 audit-relic
```

---

You are executing **batch 9** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b09` (branch `audit-relic-b09`)
...
- **`lucky_fysh`** — Sweep B: `IsAllowed` unmodelled (16-relic cluster). Also a Sweep C stub (`AfterCardChangedPiles` -> `after_card_added_to_deck` is dispatched).
- **`massive_scroll`** — Sweep B verified CLEAN (`Players.Count > 1`, never allowed single-player; absent from the grab bag; Neow already gates it). Confirm, don't re-litigate.
- **`meal_ticket`** — Sweep B: `IsAllowed` unmodelled (16-relic cluster). Also a Sweep C stub (`AfterRoomEntered` is dispatched).
- **`membership_card`** — Sweep C stub (`ModifyMerchantPrice` — no matching base hook; larger fix).
- **`molten_egg`** — Sweep B: `IsAllowed` unmodelled (16-relic cluster).
...
- `audit/tools/relic_probes_b09.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-09-lessons.md` — your report
...
`audit/tools/relic_probes_b09.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b09.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-09-lessons.md`
...
- Commit on `audit-relic-b09` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b09.py .superpowers/sdd/relic-batch-09-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-09-lessons.md` containing: the 15
