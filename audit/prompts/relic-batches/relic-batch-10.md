# Relic content audit — batch 10 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b10 -b audit-relic-b10 audit-relic
```

---

You are executing **batch 10** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b10` (branch `audit-relic-b10`)
...
2. `audit/tools/PROMPT.md` (**v5**) — the versioned instruction sheet and the
...
- **`music_box`** — Sweep E (shallow clone): a sim rebuild site — enchantment/affliction/energy-cost modifiers drop. Also in Sweep A's turn-boundary bucket (`_card_being_played`, `used_this_turn`).
- **`mystic_lighter`** — Sweep C stub (`ModifyDamageAdditive` — a COMBAT hook; check `HookSystem` for the counterpart).
- **`neows_talisman`** — Sweep D: UNGUARDED `Card.upgrade()` at `neows_talisman.py:29` in `after_obtained`. Settle by execution.
- **`nunchaku`** — Sweep A flagged this for a SECOND LOOK: `_attacks_played` reads per-combat but C# has no boundary reset. Decide deliberately.
- **`old_coin`** — Sweep B: `IsAllowed` unmodelled (16-relic cluster). Also a Sweep C stub — `AfterObtained` IS dispatched (`run.py:552`), so 'no gold system' is FALSE.
- **`orichalcum`** — Named witness in `audit/records/seam/turn_structure.json` G12 (two-phase VeryEarly snapshot). Cite and match.
- **`orrery`** — Sweep C stub: `AfterObtained` dispatched.
...
- `audit/tools/relic_probes_b10.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-10-lessons.md` — your report
...
`audit/tools/relic_probes_b10.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b10.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-10-lessons.md`
...
- Commit on `audit-relic-b10` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b10.py .superpowers/sdd/relic-batch-10-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-10-lessons.md` containing: the 15
