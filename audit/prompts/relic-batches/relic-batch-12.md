# Relic content audit — batch 12 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b12 -b audit-relic-b12 audit-relic
```

---

You are executing **batch 12** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b12` (branch `audit-relic-b12`)
...
- **`pen_nib`** — Sweep A flagged this for a SECOND LOOK (`_attacks_played`, `_card_to_double`). Also a ported witness in `audit/records/seam/hook_dispatch.json` G4 — cite and match under rule 3.
- **`permafrost`** — Sweep A flagged this for a SECOND LOOK (`_activated`).
- **`planisphere`** — Sweep B: `IsAllowed` unmodelled (16-relic cluster). Also a Sweep C stub (`AfterRoomEntered` is dispatched).
...
- `audit/tools/relic_probes_b12.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-12-lessons.md` — your report
...
`audit/tools/relic_probes_b12.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b12.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-12-lessons.md`
...
- Commit on `audit-relic-b12` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b12.py .superpowers/sdd/relic-batch-12-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-12-lessons.md` containing: the 15
