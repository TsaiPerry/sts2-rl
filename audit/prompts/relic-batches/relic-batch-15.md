# Relic content audit — batch 15 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b15 -b audit-relic-b15 audit-relic
```

---

You are executing **batch 15** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b15` (branch `audit-relic-b15`)
...
- **`spiked_gauntlets`** — Named witness in `audit/records/seam/hook_dispatch.json` G2/G3 (early-phase `TryModifyEnergyCostInCombat`). Cite and match.
- **`sturdy_clamp`** — Named witness in `audit/records/seam/turn_structure.json` G1 (the block-clear PREVENTER). Cite and match.
- **`sword_of_stone`** — Sweep A candidate (`elites_defeated`; C# resets at `AfterCombatVictory`).
...
- `audit/tools/relic_probes_b15.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-15-lessons.md` — your report
...
`audit/tools/relic_probes_b15.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b15.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-15-lessons.md`
...
- Commit on `audit-relic-b15` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b15.py .superpowers/sdd/relic-batch-15-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-15-lessons.md` containing: the 15
