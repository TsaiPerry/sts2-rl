# Relic content audit — batch 18 of 18
...
git worktree add /c/Users/Perry/Desktop/sts2-rl-relic-b18 -b audit-relic-b18 audit-relic
```

---

You are executing **batch 18** of the relic content audits in an
...
WORKTREE (all work here): `c:\Users\Perry\Desktop\sts2-rl-relic-b18` (branch `audit-relic-b18`)
...
- **`wongos_mystery_ticket`** — Sweep A candidate (`combats_finished`, `gave_relic`; C# resets at `AfterCombatEnd`).
...
- `audit/tools/relic_probes_b18.py` — **your own** probe module (see below)
- `.superpowers/sdd/relic-batch-18-lessons.md` — your report
...
`audit/tools/relic_probes_b18.py` with its own `main()`; run it as
`py audit/tools/relic_probes_b18.py`. Do NOT register anything in the
...
shape surfaces, write it to `.superpowers/sdd/relic-batch-18-lessons.md`
...
- Commit on `audit-relic-b18` only. **Never push. Never touch `main` or
  `audit-relic`.**
...
5. `git add audit/records/relic audit/tools/relic_probes_b18.py .superpowers/sdd/relic-batch-18-lessons.md`
   then commit, naming the units, the gap count, and each LIVE gap with its
   one-line executed evidence.

## Report

Write `.superpowers/sdd/relic-batch-18-lessons.md` containing: the 15
