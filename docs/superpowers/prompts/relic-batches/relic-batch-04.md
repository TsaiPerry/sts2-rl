...
- **`daughter_of_the_wind`** — Named witness in `audits/seam/hook_dispatch.json` G8 (executed). Cite and match.
...
- `tools/audit/relic_probes_b04.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b04.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b04.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b04.py .superpowers/sdd/relic-batch-04-lessons.md`
