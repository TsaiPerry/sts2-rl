...
- **`horn_cleat`** — Named witness in `audits/seam/turn_structure.json` G1 (an `AfterBlockCleared` listener). Cite and match.
...
- `tools/audit/relic_probes_b07.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b07.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b07.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b07.py .superpowers/sdd/relic-batch-07-lessons.md`
