...
- **`orichalcum`** — Named witness in `audits/seam/turn_structure.json` G12 (two-phase VeryEarly snapshot). Cite and match.
...
- `tools/audit/relic_probes_b10.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b10.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b10.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b10.py .superpowers/sdd/relic-batch-10-lessons.md`
