...
- **`pen_nib`** — Sweep A flagged this for a SECOND LOOK (`_attacks_played`, `_card_to_double`). Also a ported witness in `audits/seam/hook_dispatch.json` G4 — cite and match under rule 3.
...
- `tools/audit/relic_probes_b12.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b12.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b12.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b12.py .superpowers/sdd/relic-batch-12-lessons.md`
