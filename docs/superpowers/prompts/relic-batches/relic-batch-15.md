...
- **`spiked_gauntlets`** — Named witness in `audits/seam/hook_dispatch.json` G2/G3 (early-phase `TryModifyEnergyCostInCombat`). Cite and match.
- **`sturdy_clamp`** — Named witness in `audits/seam/turn_structure.json` G1 (the block-clear PREVENTER). Cite and match.
...
- `tools/audit/relic_probes_b15.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b15.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b15.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b15.py .superpowers/sdd/relic-batch-15-lessons.md`
