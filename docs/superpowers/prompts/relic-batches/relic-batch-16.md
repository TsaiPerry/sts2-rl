...
- **`throwing_axe`** — Named witness in `audits/seam/hook_dispatch.json` G4, and the replay source that makes `relic/unsettling_lamp` G1 live. Cite and match.
...
- **`unceasing_top`** — Named witness in `audits/seam/turn_structure.json` G16. Cite and match.
...
- `tools/audit/relic_probes_b16.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b16.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b16.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b16.py .superpowers/sdd/relic-batch-16-lessons.md`
