...
- **`vambrace`** — Sweep A candidate (`_used`; C# resets at BOTH boundaries). Settle by execution. Also primary sim evidence for `audits/seam/creature_card_cmds.json` G1/G2 — cite and match under rule 3.
- **`war_paint`** — Sweep C stub: `AfterObtained` dispatched.
- **`whetstone`** — Sweep C stub: `AfterObtained` dispatched.
- **`whispering_earring`** — `audits/seam/hook_dispatch.json` N3 records a CORRECTION about this relic: it reads no combat history at all, contrary to an earlier claim. Do not reinstate the error.
...
- **`venerable_tea_set`** — Sweep A (rewritten) FROZEN CONSTRUCTOR STATE: its whole
  ...
  `audits/relic/fake_venerable_tea_set.json`, do not re-derive.
...
- `tools/audit/relic_probes_b17.py` — **your own** probe module (see below)
...
`tools/audit/relic_probes_b17.py` with its own `main()`; run it as
`py tools/audit/relic_probes_b17.py`. Do NOT register anything in the
...
5. `git add audits/relic tools/audit/relic_probes_b17.py .superpowers/sdd/relic-batch-17-lessons.md`
