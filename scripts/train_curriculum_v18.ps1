<#
v18 lean-restore run: one stage on top of the v16 s19 policy (NOT v17 s20 --
the bare-bones settle spent 15M steps decaying propped behaviors and its
decay had not plateaued; its product is the verdict table, not the policy).
Reward function = v16 minus every term v17 proved dead, per the filled
recommendation table in docs/superpowers/plans/v17-run-log.md.

  Stage  Env  Asc  Steps  Notes
  s21    run   10    15M  continue runs/sts2_run_torch_v16_s19.pt (--resume
                          handoff, same kind, NO warm start). KEPT (v17
                          proved load-bearing): floor-by-act 1/1.5/2, win 12,
                          upgrade 1.5 + knee-cap, elite 2 / boss 3 /
                          attempt 1, hp-potential 4.0, energy-waste 0.02.
                          DROPPED (v17 proved internalized/null): all potion
                          terms (k/expiry/death-penalty), potion-ent-coef,
                          remove 0.25, relic 0.25, BOTH deck injections
                          (train dist == eval dist stays; the take-rate
                          overcorrection was the injections, so the
                          conditional-energy contingency is moot). Trainer
                          machinery unchanged: --gae-lambda 0.98,
                          --aux-hp-coef 0.25, ent 0.01, lr 3e-4.
                          --critic-warmup 8: reward function changed.

Pre-registered reads (post-run, human-read, vs the v16 potfix baselines):
  * rest-upgrade share should RECOVER toward ~0.46 (it collapsed to 0.19
    bare); failure to recover on re-add is news about the heal attractor.
  * potions should HOLD ~5.7/ep unpaid (v17's internalization verdict is
    the bet this script makes; collapse = that verdict was wrong).
  * energy unspent/turn: do NOT read mid-run (v16 lesson -- it read the
    wrong direction at 29% of stage and finished at an ATH).
  * asc-0 win should return to the 6-8% band with win 12 restored.
Boss-wall diagnostic (which boss at floor 16, entry HP, deck state) is a
separate instrumented eval, not part of this script; the wall lever it
informs is v18.5/v19 scope.

No rest mask, no potion mask, ever.

  .venv\Scripts\python.exe -m pytest -q     # green before launching
  .\train_curriculum_v18.ps1                # real run; auto-evals s21
  .\train_curriculum_v18.ps1 -Smoke         # 65536 steps, scratch tag
  .\train_curriculum_v18.ps1 -Resume        # continue an interrupted run
#>
param(
    [long]$S21Steps = 15000000,
    [string]$Device = "cuda",
    [string]$Tag = "v18",
...
$ckpt = @{ 21 = Join-Path $runs "sts2_run_torch_${Tag}_s21.pt" }

if ((Test-Path $ckpt[21]) -and -not $Resume -and -not $Smoke) {
    Write-Host "$($ckpt[21]) already exists." -ForegroundColor Red
...
# vs the v16 potfix baselines (pre-registered list in the header comment).

# ── s21: lean restore — v16 rewards minus the v17-proven-dead terms ────────
Invoke-Stage -Name "s21-run-asc10-lean-restore" -SaveCkpt $ckpt[21] -PrevCkpt $SeedCkpt `
    -Steps $S21Steps -CriticWarmup 8 -EntCoef 0.01 -StageArgs (@(
    "--env", "run", "--ascension", "10", "--lr", "3e-4") + $runRewards + $longHorizon)

if (Test-Path (Join-Path $root "runs/eval_${Tag}_s21_asc10.episodes.csv")) {
    Write-Host "s21-eval-asc10 already recorded - skipping." -ForegroundColor DarkGray
} else {
    Invoke-Eval -Name "s21-eval-asc10" -Ckpt $ckpt[21] -Asc 10 -Episodes 150 `
        -Csv "runs/eval_${Tag}_s21_asc10"
}
if (Test-Path (Join-Path $root "runs/eval_${Tag}_s21_asc0.episodes.csv")) {
    Write-Host "s21-eval-asc0 already recorded - skipping." -ForegroundColor DarkGray
} else {
    Invoke-Eval -Name "s21-eval-asc0" -Ckpt $ckpt[21] -Asc 0 -Episodes 150 `
        -Csv "runs/eval_${Tag}_s21_asc0"
}

Write-Host "v18 lean-restore run complete. Reads: docs/superpowers/plans/v18-run-log.md" -ForegroundColor Green
