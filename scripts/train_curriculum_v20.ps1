<#
v20 boss/elite drill stage (plan: docs/superpowers/plans/
2026-08-19-v20-drill-env-plan.md). Seeds the standing checkpoint v18_s21
(v19_s22 was dominated and deleted) and changes TWO things, deliberately
(the plan's attribution note says how the reads separate them):
  1. Start-state distribution: --drill-prob 0.5 of episodes begin at a
     harvested mid-run combat (schema-2 bank, runs\snapshots\v20_bank.jsonl
     from harvest_banks.ps1), stratified over five equal pools -- a1boss /
     a2boss / a3boss / a2elite / a3elite -- with the two weakest bosses
     (Vantom 38% win, Kaiser Crab 17% at asc-10 per the 2026-08-19 boss
     diagnostic) oversampled x2 within their pools. Drill episodes CONTINUE
     TO NATURAL END (locked decision 2: same state must have same value).
  2. --boss-hp-loss-penalty 2: non-refundable price on HP lost inside a
     BOSS combat, paid once at combat resolution. Exists because the
     act-entry heal refunds the hp-potential term's boss-fight losses --
     without this the policy has no gradient toward tighter boss play
     (won fights currently eat 7.5 dmg/turn with 45% zero-block turns).

  Stage  Env  Asc  Steps  Notes
  s23    run   10    15M  continue runs/sts2_run_torch_v18_s21.pt. Rewards =
                          v18's lean set + boss-hp-loss 2. --critic-warmup 8:
                          drill episodes start mid-run, so per-episode return
                          scale shifts (shorter horizons on half the batch).

Pre-registered reads (post-run, human-read; baselines = eval_v18_s21_* and
the 2026-08-19 boss_probe tables -- REBASELINE THE ELITE PROBE ON v18_s21
BEFORE READING s23's, per plan Task 6):
  * PRIMARY -- per-boss win rates (boss_probe): Vantom 38% -> >=55%,
    Kaiser Crab 17% -> >=35% at asc-10; act-3 trio 0/12 -> any wins.
  * asc-10 floor: HOLD >= 21.4 (10% below v18's 23.73). At drill-prob 0.5
    this fork is LIVE: only half the episodes practice floors 0-15 --
    a sag says halve drill-prob to 0.25 before touching anything else.
  * asc-0 win: beat 8% (boss conversion is the direct bottleneck).
  * boss_hp_lost / dmg-per-turn IN WON FIGHTS: down from 7.5 (the Task-3b
    term's own read -- drills alone have no reason to move it). Failure
    signature to watch: boss fight LENGTH down together with win rate =
    the penalty taught racing, not blocking.
  * take rate / rest-upgrade share: hold (run-skill erosion tripwires).
  * elites_fought/ep NOT down (elite-shyness check; attempt-pay guards it).

No rest mask, no potion mask, ever.

  .\harvest_banks.ps1                       # build the bank first (hours)
  .venv\Scripts\python.exe -m pytest -q     # green before launching
  .\train_curriculum_v20.ps1                # real run; auto-evals s23
  .\train_curriculum_v20.ps1 -Smoke         # 65536 steps, scratch tag
  .\train_curriculum_v20.ps1 -Resume        # continue an interrupted run
#>
param(
    [long]$S23Steps = 15000000,
    [string]$Device = "cuda",
    [string]$Tag = "v20",
    [string]$SeedCkpt = "runs/sts2_run_torch_v18_s21.pt",
    [string]$DrillBank = "runs/snapshots/v20_bank.jsonl",
    [double]$DrillProb = 0.5,
    [switch]$Resume,
    [switch]$Smoke
)
...
$ckpt = @{ 23 = Join-Path $runs "sts2_run_torch_${Tag}_s23.pt" }

if ((Test-Path $ckpt[23]) -and -not $Resume -and -not $Smoke) {
    Write-Host "$($ckpt[23]) already exists." -ForegroundColor Red
...
if (-not (Test-Path (Join-Path $root $SeedCkpt))) {
    ...
    Write-Host "(v20 seeds from v18 s21, the standing checkpoint - v19_s22 was deleted.)"
    exit 1
}
if (-not (Test-Path (Join-Path $root $DrillBank))) {
    Write-Host "Drill bank '$DrillBank' not found." -ForegroundColor Red
    Write-Host "Run .\harvest_banks.ps1 first (checks the per-pool coverage bars too)."
    exit 1
}
...
# v18's lean reward set, byte-identical, PLUS the one new term (Task 3b).
$runRewards = @("--floor-rewards", "1.0", "1.5", "2.0", "--reward-win", "12",
                ...
                "--boss-hp-loss-penalty", "2")

# v20 drill mix: five equal pools, weak bosses x2 within theirs (plan,
# Perry's calls 2026-08-19). kaiser_crab_boss IS Crusher+Rocket.
$drillArgs = @("--drill-snapshots", $DrillBank,
               "--drill-prob", "$DrillProb",
               "--drill-pools", "a1boss=0.2,a2boss=0.2,a3boss=0.2,a2elite=0.2,a3elite=0.2",
               "--drill-weights", "vantom_boss=2,kaiser_crab_boss=2")
...
# -WarmStart marks a kind-switch handoff (combat<->run): UNUSED in v20 -- the
# drill mode lives INSIDE the run env (no kind switch, that is the point).
# Kept verbatim so the helper stays byte-identical with the v10..v19 scripts.
...
# (pre-registered list in the header). NOTE: evals run WITHOUT drill flags --
# the gates live on the unmodified run distribution (locked decision 5).

# ── s23: lean rewards + boss-hp price, half the episodes drill-started ─────
Invoke-Stage -Name "s23-run-drill" -SaveCkpt $ckpt[23] -PrevCkpt $SeedCkpt `
    -Steps $S23Steps -CriticWarmup 8 -EntCoef 0.01 -StageArgs (@(
    "--env", "run", "--ascension", "10", "--lr", "3e-4") +
    $runRewards + $drillArgs + $longHorizon)

if (Test-Path (Join-Path $root "runs/eval_${Tag}_s23_asc10.episodes.csv")) {
    Write-Host "s23-eval-asc10 already recorded - skipping." -ForegroundColor DarkGray
} else {
    Invoke-Eval -Name "s23-eval-asc10" -Ckpt $ckpt[23] -Asc 10 -Episodes 150 `
        -Csv "runs/eval_${Tag}_s23_asc10"
}
if (Test-Path (Join-Path $root "runs/eval_${Tag}_s23_asc0.episodes.csv")) {
    Write-Host "s23-eval-asc0 already recorded - skipping." -ForegroundColor DarkGray
} else {
    Invoke-Eval -Name "s23-eval-asc0" -Ckpt $ckpt[23] -Asc 0 -Episodes 150 `
        -Csv "runs/eval_${Tag}_s23_asc0"
}

Write-Host "v20 drill run complete. PRIMARY read = boss_probe per-boss win rates" -ForegroundColor Green
Write-Host "(rebaseline the elite probe on v18_s21 BEFORE reading s23's numbers)." -ForegroundColor Green
