<#
v22 potion discard + restored ±k ledger run: one stage on top of the v18
s21 policy, migrated to head_version 5 (adds the out-of-combat potion
discard action; tools\migrate_headv5.py must be run first -- the migrated
seed already lives at runs/sts2_run_torch_v18_s21_headv5.pt). ONE change to
the v18 lean reward set vs v21: --potion-option-value/--potion-option-expiry
are GONE, replaced by --potion-potential-scale 0.5 --potion-death-expiry
(v21's option-value term dropped -- see docs/superpowers/plans/v21-run-
log.md verdict). Everything else in $runRewards is v18's lean set verbatim.

  Stage  Env  Asc  Steps  Notes
  s25    run   10    15M  continue runs/sts2_run_torch_v18_s21_headv5.pt
                          (--resume handoff, same kind, NO warm start). v18
                          lean rewards + the restored ±k potion ledger +
                          the new discard affordance.
                          --critic-warmup 8: reward function changed.

Pre-registered reads = docs/superpowers/plans/v22-run-log.md. Evals with
...
No rest mask, no potion mask, ever.
  ...
  .\train_curriculum_v22.ps1                # real run; auto-evals s25
  .\train_curriculum_v22.ps1 -Smoke         # 65536 steps, scratch tag
  .\train_curriculum_v22.ps1 -Resume        # continue an interrupted run
#>
param(
    [long]$S25Steps = 15000000,
    [string]$Device = "cuda",
    [string]$Tag = "v22",
    [string]$SeedCkpt = "runs/sts2_run_torch_v18_s21_headv5.pt",
    [switch]$Resume,
    [switch]$Smoke
)
...
$ckpt = @{ 25 = Join-Path $runs "sts2_run_torch_${Tag}_s25.pt" }

if ((Test-Path $ckpt[25]) -and -not $Resume -and -not $Smoke) {
    Write-Host "$($ckpt[25]) already exists." -ForegroundColor Red
...
# vs the v18_s21 --merge-duplicates baselines (reads table in v22-run-log.md).

# ── s25: v18 lean rewards + restored ±k potion ledger + discard affordance ──
Invoke-Stage -Name "s25-run-asc10-potion-ledger" -SaveCkpt $ckpt[25] -PrevCkpt $SeedCkpt `
    -Steps $S25Steps -CriticWarmup 8 -EntCoef 0.01 -StageArgs (@(
    "--env", "run", "--ascension", "10", "--lr", "3e-4") + $runRewards + $longHorizon)

foreach ($asc in 10, 0) {
    if (Test-Path (Join-Path $root "runs/eval_${Tag}_s25_asc${asc}.episodes.csv")) {
        Write-Host "s25-eval-asc$asc already recorded - skipping." -ForegroundColor DarkGray
    } else {
        Invoke-Eval -Name "s25-eval-asc$asc" -Ckpt $ckpt[25] -Asc $asc -Episodes 150 `
            -Csv "runs/eval_${Tag}_s25_asc$asc"
    }
}
Write-Host "v22 potion discard+ledger run complete. Reads: docs/superpowers/plans/v22-run-log.md" -ForegroundColor Green
