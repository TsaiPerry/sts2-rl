...
sites, `monsters/glory/aeonglass.py`'s `_AeonglassWitherListener` is the
sim's first ported listener for it, and the two former open-coded upgrade
...
from sts2_rl.monsters.glory.aeonglass import _AeonglassWitherListener
...
def test_boss_own_wither_arrives_fake_upgraded_via_the_hook():
    ...
    site -- it relies on `_AeonglassWitherListener` firing off
...
def test_listener_ignores_a_card_with_no_upgrade_count_pending():
    """Direct unit check on the listener class: zero pending upgrades is a
    no-op (range(0)), matching a freshly-spawned Aeonglass before its first
    Increasing Intensity move."""
    cs = fresh_with(Aeonglass)
    boss = cs.enemies[0]
    assert boss.wither_upgrade_count == 0
    listener = _AeonglassWitherListener(boss)
    wither = make_card("wither")
    listener.on_card_generated_for_combat(wither)
    assert wither.fake_upgrade_level == 0


def test_aeonglass_registers_the_wither_listener_on_construction():
    cs = fresh_with(Aeonglass)
    boss = cs.enemies[0]
    assert any(isinstance(l, _AeonglassWitherListener) and l.aeonglass is boss
              for l in cs.hooks._listeners)
